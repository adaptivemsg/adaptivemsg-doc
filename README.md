...
- `adaptivemsg-rust` (crate name stays `adaptivemsg`)
  - Runtime + macros + Rust → Go generator (`cmd/amgen/`).
- `adaptivemsg-go`
  - Go runtime + Go → Rust generator (`cmd/amgen-go/`).
- `adaptivemsg-doc`
  - This proposal.
...
- `adaptivemsg-go`: Go runtime (module `github.com/adaptivemsg/adaptivemsg-go`).
  - `cmd/amgen-go/`: Go → Rust generator (via `go generate`).
...
- Rust -> Go: use the Rust `amgen-rs` binary (install with `cargo install --git https://github.com/adaptivemsg/adaptivemsg-rust.git adaptivemsg-amgen`).
...
hello-server-go/
  go.mod
  Cargo.toml        # generated by amgen-go
  lib.rs            # generated by amgen-go: exposes api module
  api/
    hello/
      message.go      # Go message structs (source of truth)
      message.rs      # generated by amgen-go
```
...
`amgen-go` generates three files:

- `api/hello/message.rs` — Rust type definitions (flat, no module wrapper)
- `lib.rs` — module tree preserving the `api/<service>/` directory structure
- `Cargo.toml` — crate manifest with `[lib] path = "lib.rs"`

It refuses to overwrite an existing `Cargo.toml`. After generation, set the
`adaptivemsg` dependency to a git source in the generated `Cargo.toml`.

Generated `lib.rs` example (for `api/hello/message.rs`):
...
# Cargo.toml
[dependencies]
hello = { git = "https://github.com/adaptivemsg/hello-server-go.git" }
```

```rust
use hello::api::hello::{HelloRequest, HelloReply};
```
...
```
hello-server-rust/
  Cargo.toml        # Rust server crate
  go.mod            # Go module (exposes generated Go types)
  lib.rs            # Rust lib entry point
  server.rs         # Rust server binary
  api/
    hello/
      message.rs      # Rust message structs (source of truth)
      handler.rs      # Rust handler implementation
      message.go      # generated by amgen-rs
```

Generate Go types from Rust:

```
amgen-rs api/hello/message.rs
```

This generates `api/hello/message.go` alongside the source `.rs` file.

Go client usage:

```go
import hello "github.com/adaptivemsg/hello-server-rust/api/hello"
```

```go
reply := &hello.HelloReply{}
err := conn.SendRecv(&hello.HelloRequest{Who: "John", Question: "who are you"}, reply)
```
